...
impl UdpSocket {
    ...
    pub async fn bind<A: ToSocketAddrs>(addr: A) -> Result<UdpSocket> {
        World::current(|world| {
            ...
            verify_bind_address(addr.ip(), host.addr)?;
    ...
    pub fn join_multicast_v4(&self, multiaddr: Ipv4Addr, interface: Ipv4Addr) -> Result<()> {
        ...
        World::current(|world| {
            ...
            verify_bind_address(interface, dst.ip())?;
    ...
    pub fn join_multicast_v6(&self, multiaddr: &Ipv6Addr, _interface: u32) -> Result<()> {
        if !multiaddr.is_multicast() {
            return Err(Error::new(ErrorKind::InvalidInput, "Invalid argument"));
        }
    ...
    pub fn leave_multicast_v6(&self, multiaddr: &Ipv6Addr, _interface: u32) -> io::Result<()> {
        if !multiaddr.is_multicast() {
            return Err(Error::new(ErrorKind::InvalidInput, "Invalid argument"));
        }
...
fn verify_bind_address<A>(addr: A, host: IpAddr) -> Result<()>
where
    A: Into<IpAddr>,
{
    let addr = addr.into();
    ...
    if addr.is_ipv4() != host.is_ipv4() {
        panic!("ip version mismatch: {:?} host: {:?}", addr, host)
    }

    Ok(())
}

fn destination_address(world: &World, socket: &UdpSocket) -> io::Result<SocketAddr> {
    let local_port = socket.local_addr()?.port();
    let host_addr = world.current_host().addr;
    Ok(SocketAddr::from((host_addr, local_port)))
}

impl Drop for UdpSocket {
    fn drop(&mut self) {
        World::current_if_set(|world| {
            if let Ok(dst) = destination_address(world, self) {
                world.multicast_groups.leave_all(&dst);
            };
...
mod tests {
    ...
    mod multicast_group {
        ...
        fn joining_does_not_produce_duplicate_addresses() {
            ...
            let addrs = groups.0.values().flat_map(|addr| addr).collect::<Vec<_>>();
