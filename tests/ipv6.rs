...
#[test]
fn ipv6_lookup() {
    let mut sim = Builder::new().network(IpNetwork::V6).build();
...
fn ipv6_udp_bind_panic_at_addr_missmatch() {
    let mut sim = Builder::new().network(IpNetwork::V6).build();
    sim.client("client", async move {
        let _sock = UdpSocket::bind("0.0.0.0:0").await.unwrap();
        Ok(())
    });
    sim.run().unwrap()
}
...
fn ipv4_udp_bind_panic_at_addr_missmatch() {
    let mut sim = Builder::new().network(IpNetwork::V4).build();
    sim.client("client", async move {
        let _sock = UdpSocket::bind(":::0").await.unwrap();
        Ok(())
    });
    sim.run().unwrap()
}
...
fn ipv6_tcp_bind_panic_at_addr_missmatch() {
    let mut sim = Builder::new().network(IpNetwork::V6).build();
    sim.client("client", async move {
        let _sock = TcpListener::bind("0.0.0.0:1000").await.unwrap();
        Ok(())
    });
    sim.run().unwrap()
}
...
fn ipv4_tcp_bind_panic_at_addr_missmatch() {
    let mut sim = Builder::new().network(IpNetwork::V4).build();
    sim.client("client", async move {
        let _sock = TcpListener::bind(":::1000").await.unwrap();
        Ok(())
    });
    sim.run().unwrap()
}
